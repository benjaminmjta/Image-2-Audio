...
def image_to_audio(input_image, output, color_depth):
    """
    ...
    bit2audio_optimized(bits, output)


def audio_to_image(input_audio, output, width, height, color_depth , ft_version = 0):
    """
    ...
    :return: None
    """
    bits = audio2bit(input_audio, ft_version)
    bit2img(bits, output, width, height, color_depth)
...
def img2bit(input_image, color_depth):
    ...
    width, height = image.size
    size = width * height
    max_size = v.image_max_size

    if size > max_size:
        resize_img(input_image, max_size)
        image = Image.open(input_image)

    # -> excluded too much error rate
    # width, height = image.size
    ...
    ## bitstring = height_bits + width_bits + color_depth_bits + pixel_bits -> excluded too much error rate

    bitstring = pixel_bits
    print(f"Image {input_image} successfully encoded to bitstring.")
    return bitstring


def bit2audio(bitstring, encoded_audio):
    """
    ...
    :return: None
    """
    frequencies = [500 + i * v.audio_symbol_frequency_distance for i in range(16)]
    amplitude = 32767
    samples_per_bitgroup = v.audio_symbol_duration * v.audio_sample_rate
    frequency_signals = {}
    for i in frequencies:
        frequency_signals[i] = [
            int(amplitude * math.sin(2 * math.pi * i * t / v.audio_sample_rate))
    ...
    marker_samples = int(v.audio_symbol_start_duration * v.audio_sample_rate)
    startmarker_wave = [
        int(amplitude * math.sin(2 * math.pi * v.audio_symbol_start_frequency * i / v.audio_sample_rate))
    ...
    with wave.open(encoded_audio, 'wb') as wf:
        wf.setnchannels(1)
        wf.setsampwidth(2)
        wf.setframerate(v.audio_sample_rate)
...
def bit2audio_optimized(bitstring, encoded_audio):
    """
    converts bitstring to audio (.wav)
    :param bitstring: bitstring of the image, has to be formatted as follows: height_bits(16bits) + width_bits(16bits) + color_depth_bits(8bits) + pixel_bits
    :param encoded_audio: path and filename of the output encoded audio file
    :return: None
    """
    amplitude = v.audio_amplitude
    sample_rate = v.audio_sample_rate
    symbol_duration = v.audio_symbol_duration
    symbol_frequency_distance = v.audio_symbol_frequency_distance
    startmarker_frequency = v.audio_symbol_start_frequency
    startmarker_duration = v.audio_symbol_start_duration

    frequencies = np.array([500 + i * symbol_frequency_distance for i in range(16)], dtype=np.float32)
    samples_per_bitgroup = int(symbol_duration * sample_rate)
...
def find_startmarker(signal, accuracy = 0.01):
    """
    ...
    """
    window_size = int(v.audio_sample_rate * accuracy)
    ...
    for i in range(0, len(signal) - window_size, step_size):
        ...
        freqs = np.fft.fftfreq(len(segment), 1 / v.audio_sample_rate)
        magnitude = np.abs(fft_result)

        dominant_freq = abs(freqs[np.argmax(magnitude)])

        if (abs(dominant_freq - v.audio_symbol_start_frequency) <= 10) and (dominant_freq > 0):
            start_sample = int(i + (v.audio_symbol_start_duration * v.audio_sample_rate))
            break

    return start_sample


def audio2bit(encoded_audio, ft_version):
    """
    ...
    if framerate != v.audio_sample_rate:
        raise ValueError("sample rate of wav file must be 44100 Hz.")

    signal = np.frombuffer(audio_data, dtype=np.int16)

    start_index = find_startmarker(signal)
    print(f"startmarker found, start at {start_index} samples or {start_index / v.audio_sample_rate} seconds.")
    if start_index == -1:
        raise ValueError(
            "startmarker not found, audio cannot be decoded."
        )

    samples_per_symbol = int(v.audio_sample_rate * v.audio_symbol_duration)
    ...
    for i in range(start_index, len(signal), samples_per_symbol):
        ...
        if closest == frequencies[16]:
            print(f"startmarker frequency, skipping symbol.")
            continue
        ...
        if dominant_frequency in frequencies:
            bitstring += format(int(dominant_frequency/v.audio_symbol_frequency_distance) - int(frequencies[0]/v.audio_symbol_frequency_distance), '04b')
        else:
...
def bit2img(bitstring, output_image, width = 64, height = 64, color_depth = 1):
    """
    converts bitstring to image
    :param color_depth: depth of color information min: 2 max: 8
    ...
    print(f"width: {width}, height: {height}, color_depth: {color_depth}")

    if width * height * color_depth != len(bitstring):
        bitstring = bitstring[:(width * height * color_depth)]

    bits_per_pixel = color_depth
    pixel_bits = bitstring
    pixels = []
